...
(function () {
  ...
  const btnFecharAdmin = $("#admin-fechar");
  ...
  function abrirAdmin() {
    ...
    loginView.hidden = ativa;
    painelView.hidden = !ativa;
    if (ativa) {
      carregarPainelAdmin();
    } else {
      setTimeout(() => $("#admin-usuario").focus(), 50);
    }
  }
  ...
  modalAdmin.addEventListener("click", (e) => { if (e.target === modalAdmin) fecharAdmin(); });
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && !modalAdmin.hidden) fecharAdmin();
  });
